...
#AGE BRACKET, clean up

for (age in sc$AgeBracket){
  if (grepl("[+]", age) == TRUE){
    age = strsplit(age, '+')
    age = unlist(age)
    age = glue::glue("{age[1]}{age[2]}")
    age = as.list(seq(age, 50, by = 1))
...
as.numeric(sc$AgeBracket)

sc$AgeBracket = as.character(sc$AgeBracket)
sc$AgeBracket = gsub("-",":",sc$AgeBracket)
lapply(sc$AgeBracket, seq())
#sc$AgeBracket = strsplit(sc$AgeBracket, ":")

for (age in 1:length(sc$AgeBracket)){
  sc$AgeBracket[age] = unlist(strsplit(sc$AgeBracket[age], '-'))
  sc$AgeBracket[age] = list(seq(sc$AgeBracket[age][1],sc$AgeBracket[age][2]))
}
